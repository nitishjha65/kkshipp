...
export default function PostsCards({ details, onClick }: PostsCardsProps) {
  const { push } = useRouter();

  const [isPostDetails, setIspostdetails] = useState<boolean>(false);
  ...
  const handleCardClick = () => {
    //set boolean true for postDetails
    setIspostdetails(true);

    // if (details?._id) {
    //   push(`/profile/postDetails/${details?._id}`);
    // }
  };
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className="text-xs border border-gray-300 rounded px-2 py-0.5 whitespace-nowrap"
                    // onClick={() => push(`profile/${details?.userId}`)}
                  >
                    {details?.userLevel}
                  </span>
