...
export default function CourseDetails({
  ...
  const [isJoined, setIsJoined] = useState(details?.joined ?? false);
    ...
    if (token) {
      try {
        ...
        if (
          ...
          response?.data?.message === "Successfully joined the session"
        ) {
          setIsJoined(true); // Update the button state
        } else {
          setIsJoined(false); // Update the button state if any other message
        }
      } catch (error: any) {
        console.log("Error toggling join status:", error);
  ...
  console.log("csdvcs", isNotLoggedIn);

  console.log("hideJoiyyyyyyyyyyyyyyyyyyyyyyyyn", hideJoin);
  return (
    <>
